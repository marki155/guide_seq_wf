...
class GuideSeq:
    ...
    def parseManifest(self, manifest_path):
        ...
        try:
            ...
            LOG.error('Incorrect or malformed manifest file. Please ensure your manifest contains all required fields.')
            sys.exit()
        ...
        LOG.info('Successfully loaded manifest.')

    def parseManifestDemultiplex(self, manifest_path):
        LOG.info('Loading manifest for demultiplexing...')
        ...
        with open(manifest_path, 'r') as f:
            ...
            try:
                ...
                LOG.error('Incomplete or incorrect manifest file. Please ensure your manifest contains all required fields.')
                quit()
        ...
        LOG.info('Successfully loaded manifest for single-step demultiplexing.')

    def demultiplex(self):

        LOG.info('Demultiplexing undemultiplexed files...')
        ...
        try:
            ...
            LOG.info('Successfully demultiplexed reads.')
        except Exception as e:
            LOG.error('Error demultiplexing reads.')
            LOG.error(traceback.format_exc())
            quit()

    def umitag(self):
        LOG.info('umitagging reads...')
        ...
        try:
            ...
            LOG.info('Successfully umitagged reads.')
        except Exception as e:
            LOG.error('Error umitagging')
            LOG.error(traceback.format_exc())
            quit()

    def consolidate(self, min_freq=CONSOLIDATE_MIN_FREQ, min_qual=CONSOLIDATE_MIN_QUAL):
        LOG.info('Consolidating reads...')
        ...
        try:
            ...
            LOG.info('Successfully consolidated reads.')
        except Exception as e:
            LOG.error('Error umitagging')
            LOG.error(traceback.format_exc())
            quit()
    ...
    def alignReads(self):
        ...
        try:
            ...
            for sample in self.samples:
                ...
                LOG.info('Finished aligning reads to genome.')

        except Exception as e:
            LOG.error('Error aligning')
            LOG.error(traceback.format_exc())
            quit()

    def identifyOfftargetSites(self):
        LOG.info('Identifying offtarget sites...')
        ...
        try:
            ...
            LOG.info('Finished identifying offtarget sites.')

        except Exception as e:
            LOG.error('Error identifying offtarget sites.')
            LOG.error(traceback.format_exc())
            quit()

    def filterBackgroundSites(self):
        LOG.info('Filtering background sites')
        ...
        try:
            ...
            for sample in self.samples:
                if sample != 'control':
                    ...
                    LOG.info('Finished background filtering for {0} sample'.format(sample))

            LOG.info('Finished filtering background sites.')

        except Exception as e:
            LOG.error('Error filtering background sites.')
            LOG.error(traceback.format_exc())

    def visualize(self):
        LOG.info('Visualizing off-target sites')
        ...
        try:
            ...
            LOG.info('Finished visualizing off-target sites')

        except Exception as e:
            LOG.error('Error visualizing off-target sites.')
            LOG.error(traceback.format_exc())
